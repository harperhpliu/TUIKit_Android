...
class AudioRecorderSystem(private val context: Context) : AudioRecorderInternalInterface {
    ...
    override fun startRecord(filePath: String?, minRecordDurationMs: Int, maxRecordDurationMs: Int) {
        if (isEnableAIDeNoise && context.isAppDebuggable()) {
            Toast.makeText(context, R.string.video_recorder_audio_authorization_prompter, Toast.LENGTH_SHORT).show()
        }
        ...
        try {
            ...
            mediaRecorder?.apply {
                ...
                try {
                    ...
                    listener?.onCompleted(AudioRecorderResultCode.ERROR_RECORD_INNER_FAIL, " ")
                    reset()
                    release()
                    mediaRecorder = null
                }
    ...
    override fun stopRecord() {
        ...
        try {
            ...
            if (recordTime < minRecordDurationMs) {
                listener?.onCompleted(AudioRecorderResultCode.ERROR_LESS_THAN_MIN_DURATION, filePath)
            } else if (recordTime >= maxRecordDurationMs) {
                listener?.onCompleted(AudioRecorderResultCode.SUCCESS_EXCEED_MAX_DURATION, filePath)
            } else {
                listener?.onCompleted(AudioRecorderResultCode.SUCCESS, filePath)
            }
        } catch (e: Exception) {
            listener?.onCompleted(AudioRecorderResultCode.ERROR_RECORD_INNER_FAIL, "")
            releaseResources()
