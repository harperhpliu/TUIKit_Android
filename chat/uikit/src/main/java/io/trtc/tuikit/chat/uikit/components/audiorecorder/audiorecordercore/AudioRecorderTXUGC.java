...
import io.trtc.tuikit.chat.uikit.components.audiorecorder.audiorecordercore.TXUGCAudioRecorderReflector.AudioConfig;
import io.trtc.tuikit.chat.uikit.components.audiorecorder.audiorecordercore.TXUGCAudioRecorderReflector.RecordResult;
import io.trtc.tuikit.chat.uikit.components.audiorecorder.audiorecordercore.TXUGCAudioRecorderReflector.TXUGCAudioRecorderReflectorListener;
...
public class AudioRecorderTXUGC implements AudioRecorderInternalInterface, TXUGCAudioRecorderReflectorListener {
    ...
    public void startRecord(@Nullable String filePath, int minRecordDurationMs, int maxRecordDurationMs) {
        ...
        if (result == START_RECORD_ERR_LICENCE_VERIFICATION_FAILED) {
            ...
            mListener.onCompleted(AudioRecorderResultCode.ERROR_RECORD_INNER_FAIL, "");
        }
    }
    ...
    public void onComplete(RecordResult result) {
        ...
        if (result.retCode == ERROR_LESS_THAN_MIN_DURATION) {
            mListener.onCompleted(AudioRecorderResultCode.ERROR_LESS_THAN_MIN_DURATION, result.videoPath);
        } else if (result.retCode == SUCCESS_EXCEED_MAX_DURATION) {
            mListener.onCompleted(AudioRecorderResultCode.SUCCESS_EXCEED_MAX_DURATION, result.videoPath);
        } else if (result.retCode >= 0) {
            mListener.onCompleted(AudioRecorderResultCode.SUCCESS, result.videoPath);
        } else {
            mListener.onCompleted(AudioRecorderResultCode.ERROR_RECORD_INNER_FAIL, result.videoPath);
        }
    }
    ...
    private void handleLicenceVerificationFailed() {
        ...
        if (isAppDebuggable()) {
            Toast.makeText(mContext, R.string.video_recorder_audio_authorization_prompter, Toast.LENGTH_SHORT).show();
        }
