...
public class BeautyHorizontalScrollView extends HorizontalScrollView {

    private final String TAG = BeautyHorizontalScrollView.class + "_" + hashCode();

    private DataSetObserver mObserver;
    private BeautyScrollViewAdapter mAdapter;

    public BeautyHorizontalScrollView(Context context) {
        super(context);
        initialize();

    }

    public BeautyHorizontalScrollView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initialize();
    }

    public BeautyHorizontalScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initialize();
    }

    public void setAdapter(BeautyScrollViewAdapter adapter) {
        Log.i(TAG, "set Adapter adapter = " + adapter);
        if (mAdapter != null) {
    ...
    public void setClicked(int position) {
        Log.i(TAG, "setClicked position = " + position + " old position = " + mAdapter.getSelectPosition());
    ...
    private void scrollToSelectView() {
        ...
        int visibleZoomLeft = scrollBounds.left + scrollX + VideoRecorderResourceUtils.dip2px(getContext(), 20);
        int visibleZoomRight = scrollBounds.right + scrollX - VideoRecorderResourceUtils.dip2px(getContext(), 20);
