...
import io.trtc.tuikit.chat.uikit.components.videorecorder.view.recordview.beauty.data.BeautyInnerType;
import io.trtc.tuikit.chat.uikit.components.videorecorder.view.recordview.beauty.data.BeautyItem;
import io.trtc.tuikit.chat.uikit.components.videorecorder.view.recordview.beauty.data.BeautyType;
...
public class BeautyPanelView extends RelativeLayout {
    ...
    VideoRecorderDataObserver<BeautyItem> mOnSelectedBeautyItemChanged = beautyItem -> {
        updateBeautyTab(beautyItem);
        updateSeekBar(beautyItem);
        setBeauty(beautyItem);
    };

    public BeautyPanelView(Context context, VideoRecorderRecordCore recordCore, RecordInfo recordInfo) {
        super(context);
        mContext = context;
    ...
    public void setVisibility(int visibility) {
        ...
        mRecordInfo.isShowBeautyView.set(visibility == VISIBLE);
    }
    ...
    private void initView() {
        LayoutInflater.from(mContext).inflate(R.layout.video_recorder_beauty_panel_view, this);
        initBeautySettingView();
    }

    private void initBeautySettingView() {
        mStrengthSettingSeekBar = findViewById(R.id.video_recorder_beauty_strength_seek_bar);
        mStrengthSettingSeekBar.setOnSeekBarChangeListener(mSeekBarChangeListener);
        mLayoutStrengthSetting = findViewById(R.id.video_recorder_beauty_strength_setting);

        ViewPager2 beautyPager = findViewById(R.id.video_recorder_view_pager);
        ...
        mBeautyTypeTabView = findViewById(R.id.video_recorder_beauty_type_tab_view);
    ...
    private void setTabText(@NonNull Tab tab, int position) {
        ...
        customTextView.setTextColor(
                VideoRecorderResourceUtils.getResources().getColor(R.color.video_recorder_beauty_type_text_color_normal));

        float textSize = VideoRecorderResourceUtils.getResources().getDimension(R.dimen.video_recorder_common_tab_text_size);
        customTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);

        String fontName =
                VideoRecorderResourceUtils.getString(R.string.video_recorder_common_font);
    ...
    private void changeTabSelectedStatus(Tab tab, boolean isSelected) {
        ...
        int textColorResourceId = R.color.video_recorder_beauty_type_text_color_normal;
        int textColorFontStyle = Typeface.NORMAL;
        if (isSelected) {
            textColorResourceId = R.color.video_recorder_beauty_type_text_color_selected;
            textColorFontStyle = Typeface.BOLD;
        }

        tabTextView.setTextColor(
                VideoRecorderResourceUtils.getResources().getColor(textColorResourceId));
        String fontName = VideoRecorderResourceUtils.getString(R.string.video_recorder_common_font);
    ...
    private void setBeauty(BeautyItem beautyItem) {
        ...
        int level = beautyItem.getLevel();
        Log.i(TAG, "set Beauty. beautyType " + beautyType
