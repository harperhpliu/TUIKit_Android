...
class SearchGroupPage(
    ...
    var onGroupClick: ((GroupSearchInfo) -> Unit)? = null
    ...
    init {
        ...
        headerText = TextView(context).apply {
            ...
            text = context.getString(R.string.search_category_group)
            val dp16 = dpToPx(16)
        ...
        emptyText = TextView(context).apply {
            ...
            text = context.getString(R.string.search_cannot_found_group)
    ...
    fun start(keywords: String) {
        ...
        scope.launch {
            viewModel.groupSearchResults.collectLatest { results ->
        ...
        scope.launch {
            viewModel.isSearching.collectLatest { searching ->
                ...
                if (searching) {
                    ...
                    updateResults(viewModel.groupSearchResults.value)
                }
            }
        }
        scope.launch {
    ...
    private fun updateResults(results: List<GroupSearchInfo>) {
        if (results.isEmpty() && searchQuery.isNotBlank()) {
            ...
            recyclerView.adapter = GroupListAdapter(context, results, searchQuery) {
                onGroupClick?.invoke(it)
            }
        }
    }
...
private class GroupListAdapter(
    ...
    private val groups: List<GroupSearchInfo>,
    private val keywords: String,
    private val onClick: (GroupSearchInfo) -> Unit
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.view.bindGroup(groups[position], keywords, colors, context) {
