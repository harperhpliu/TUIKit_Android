...
import io.trtc.tuikit.chat.demo.login.LoginActivity
...
class SettingsPageView @JvmOverloads constructor(
    ...
    private val appLanguageOptions by lazy {
        listOf(
            ActionItem(text = context.getString(R.string.settings_zh_hans), value = "zh"),
            ActionItem(text = context.getString(R.string.settings_zh_hant), value = "zh-Hant"),
            ActionItem(text = context.getString(R.string.settings_en), value = "en"),
            ActionItem(text = context.getString(R.string.settings_ar), value = "ar")
        )
    }

    init {
        LayoutInflater.from(context).inflate(R.layout.page_settings, this, true)

        pageHeader = findViewById(R.id.pageHeader)
        tvUserName = findViewById(R.id.tvUserName)
        tvUserId = findViewById(R.id.tvUserId)
        tvUserStatus = findViewById(R.id.tvUserStatus)
        userAvatar = findViewById<Avatar>(R.id.userAvatar).apply {
            setSize(Avatar.AvatarSize.XL)
        }

        itemTheme = findViewById(R.id.itemTheme)
        itemLanguage = findViewById(R.id.itemLanguage)
        itemAddRule = findViewById(R.id.itemAddRule)
        itemTranslateLanguage = findViewById(R.id.itemTranslateLanguage)

        switchReadReceipt = findViewById(R.id.switchReadReceipt)
        tvReadReceiptTitle = findViewById(R.id.tvReadReceiptTitle)
        tvReadReceiptDesc = findViewById(R.id.tvReadReceiptDesc)

        btnLogout = findViewById(R.id.btnLogout)

        settingsGroup1 = findViewById(R.id.settingsGroup1)
        settingsGroup2 = findViewById(R.id.settingsGroup2)
        ...
        allSpacers.addAll(listOf(
            findViewById(R.id.spacer1),
            findViewById(R.id.spacer2)
        ))
    ...
    private fun setupUserProfileClick() {
        val userProfileSection = findViewById<View>(R.id.userProfileSection)
    ...
    private fun applyThemeColors(colors: ColorTokens) {
        ...
        findViewById<View>(R.id.userProfileSection)?.setBackgroundColor(colors.bgColorOperate)
        ...
        val entryItems = listOf(itemTheme, itemLanguage, itemAddRule, itemTranslateLanguage)
        for (item in entryItems) {
            item.findViewById<TextView>(R.id.tvSettingsTitle)?.setTextColor(colors.textColorPrimary)
            item.findViewById<TextView>(R.id.tvSettingsValue)?.setTextColor(colors.textColorSecondary)
            item.findViewById<ImageView>(R.id.ivArrow)?.setColorFilter(colors.textColorTertiary)
        }
    ...
    private fun setupSettingsItems() {
        ...
        val themeDisplayName = when (savedThemeMode) {
            AppConstants.THEME_MODE_LIGHT -> context.getString(R.string.settings_theme_light)
            AppConstants.THEME_MODE_DARK -> context.getString(R.string.settings_theme_dark)
        ...
        setupEntryItem(
            itemAddRule,
            context.getString(R.string.settings_add_rule),
            context.getString(R.string.settings_allow_type_need_confirm)
        ) {
        ...
        setupEntryItem(
            ...
            context.getString(R.string.settings_translate_target_language),
    ...
    private fun setupEntryItem(
        ...
        view.findViewById<TextView>(R.id.tvSettingsTitle).text = title
        view.findViewById<TextView>(R.id.tvSettingsValue).text = value
    ...
    private fun setupReadReceiptToggle() {
        tvReadReceiptTitle.text = context.getString(R.string.settings_read_receipt)
    ...
    private fun updateReadReceiptDescription() {
        tvReadReceiptDesc.text = if (enableReadReceipt) {
            context.getString(R.string.settings_read_receipt_enabled_desc)
        } else {
            context.getString(R.string.settings_read_receipt_disabled_desc)
        }
    }
    ...
    private fun updateUserProfile(userInfo: UserProfile?) {
        ...
        tvUserStatus.text = "${context.getString(R.string.settings_self_detail_status)}：${userInfo?.selfSignature ?: ""}"
    ...
    private fun updateFriendAddRuleDisplay(allowType: AllowType?) {
        val value = when (allowType) {
            AllowType.ALLOW_ANY -> context.getString(R.string.settings_allow_type_allow_any)
            AllowType.DENY_ANY -> context.getString(R.string.settings_allow_type_deny_any)
            AllowType.NEED_CONFIRM -> context.getString(R.string.settings_allow_type_need_confirm)
            else -> context.getString(R.string.settings_allow_type_need_confirm)
        }
    ...
    private fun showThemeSelector() {
        ...
        val options = listOf(
            ActionItem(text = context.getString(R.string.settings_theme_system), value = AppConstants.THEME_MODE_SYSTEM),
            ActionItem(text = context.getString(R.string.settings_theme_light), value = AppConstants.THEME_MODE_LIGHT),
            ActionItem(text = context.getString(R.string.settings_theme_dark), value = AppConstants.THEME_MODE_DARK)
        )
    ...
    private fun showFriendAddRuleSelector() {
        val options = listOf(
            ActionItem(text = context.getString(R.string.settings_allow_type_allow_any), value = AllowType.ALLOW_ANY),
            ActionItem(text = context.getString(R.string.settings_allow_type_deny_any), value = AllowType.DENY_ANY),
            ActionItem(text = context.getString(R.string.settings_allow_type_need_confirm), value = AllowType.NEED_CONFIRM)
        )
        ActionSheet.show(context, options) { selected ->
    ...
    private fun getCurrentLanguageDisplayName(): String {
        ...
        return when {
            currentTag.isBlank() -> context.getString(R.string.settings_current_language)
            isTraditionalChinese(currentTag) -> context.getString(R.string.settings_zh_hant)
            currentTag.startsWith("zh", ignoreCase = true) -> context.getString(R.string.settings_zh_hans)
            currentTag.startsWith("en", ignoreCase = true) -> context.getString(R.string.settings_en)
            currentTag.startsWith("ar", ignoreCase = true) -> context.getString(R.string.settings_ar)
            else -> context.getString(R.string.settings_current_language)
        }
    }
