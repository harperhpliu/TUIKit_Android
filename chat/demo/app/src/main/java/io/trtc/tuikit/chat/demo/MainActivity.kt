...
class MainActivity : BaseActivity() {
    ...
    private val tabItems = listOf(
        TabItem(R.string.tab_messages, R.id.tab_messages),
        TabItem(R.string.tab_contacts, R.id.tab_contacts),
        TabItem(R.string.tab_me, R.id.tab_me)
    )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        messageUnreadBadge = findViewById(R.id.messageUnreadBadge)
    ...
    override fun onStart() {
        ...
        mainScope?.launch {
            conversationListStore.state.totalUnreadCount.collectLatest { unreadCount ->
                updateTabBadge(R.id.tab_messages, unreadCount.toInt())
            }
        }
        mainScope?.launch {
            combine(
                ...
                updateTabBadge(R.id.tab_contacts, unreadCount)
            }
        }
        mainScope?.launch {
            groupStore.groupEventFlow.collectLatest { event ->
                when (event) {
                    is GroupEvent.OnKickedFromGroup -> showGroupEventToast(
                        ...
                        messageResId = R.string.group_event_kicked
                    )
                    is GroupEvent.OnGroupDismissed -> showGroupEventToast(
                        groupID = event.groupID,
                        messageResId = R.string.group_event_dismissed
                    )
                    else -> Unit
                }
            }
    ...
    private fun clearAllUnreadByBadgeDrag(badgeView: View) {
        ...
        runCatching {
            ...
            Toast.makeText(this, R.string.clear_all_unread_success, Toast.LENGTH_SHORT).show()
        }.onFailure {
            resetMessageBadgeDrag(badgeView)
        }
    }
    ...
    private fun showGroupEventToast(groupID: String, messageResId: Int) {
        val fallbackName = groupID.ifEmpty { getString(R.string.group_event_unknown_group) }
    ...
    private fun applyTabBadgeColors(colors: ColorTokens) {
        bottomNav.getBadge(R.id.tab_contacts)?.let { badge ->
    ...
    private fun updateTabBadge(menuItemId: Int, unreadCount: Int) {
        if (menuItemId == R.id.tab_messages) {
    ...
    private fun updateMessageTabBadge(unreadCount: Int) {
        bottomNav.removeBadge(R.id.tab_messages)
        if (unreadCount <= 0) {
            if (!isDraggingMessageBadge) {
    ...
    private fun findMessageTabItemView(): View? {
        val itemIndex = tabItems.indexOfFirst { it.menuItemId == R.id.tab_messages }
        if (itemIndex < 0) {
            return null
        }
    ...
    private fun createConversationsPage(): View {
        ...
        page.setHeaderTitle(getString(R.string.page_conversations_title))
    ...
    private fun createContactsPage(): View {
        ...
        page.setHeaderTitle(getString(R.string.page_contacts_title))
        val addButton = ImageView(this).apply {
            setImageResource(io.trtc.tuikit.chat.uikit.R.drawable.uikit_ic_add_circle)
            layoutParams = ViewGroup.LayoutParams(24.dpToPx(), 24.dpToPx())
        }
        ...
        addButton.setOnClickListener { anchor ->
            PopupMenuHelper(this).show(
                ...
                listOf(
                    PopupMenuItem(
                        title = getString(R.string.add_friend),
    ...
    private fun createMePage(): View {
        ...
        page.setHeaderTitle(getString(R.string.page_me_title))
        return page
    }
